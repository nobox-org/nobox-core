...
export const redisUtils = (args: {
   ...
   const invalidateCache = async (options?: {
      ...
      logger.sLog(
         options,
         `redisUtils::${collectionName}::invalidateCache invalidating cache`,
      );
   ...
   const retrieveCache = async <R>(primaryKey: string): Promise<R> => {
      logger.sLog(
         { primaryKey },
         `redisUtils::${collectionName}::retrieveCache retrieving cache`,
      );
      const cacheValue = await redisClient.hGet(hashKey, primaryKey);
      if (cacheValue) {
         logger.sLog(
            { cacheValue },
            `redisUtils::${collectionName}::found Response from redis`,
         );
         return JSON.parse(cacheValue);
      }
      logger.sLog(
         { hashKey, primaryKey },
         `redisUtils::${collectionName}:: cache does not exist for primaryKey`,
      );
      return null;
   };
   ...
   const updateCache = async (primaryKey: string, value: string) => {
      logger.sLog(
         { hashKey, primaryKey, value },
         `redisUtils::${collectionName}::updateCache updating cache`,
      );
