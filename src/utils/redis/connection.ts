...
export const redisConnection = (logger: Logger) => {
    ...
    const connectToRedisServer = (args?: { init: boolean, restart?: boolean }) => {
        const { init = false, restart = false } = args || {};
        logger.sLog({ init }, "redisConnection:: connection To RedisServer requested");

        if (acquiringConnection && init) {
            logger.sLog({ init }, "redisConnection:: connection already in progress, initialization not needed");
            return;
        }
        ...
        try {
            ...
            client.on('error', (err) => {
                logger.sLog({ err })
                logger.sLog({}, 'Error occurred while connecting or accessing redis server');
            });
