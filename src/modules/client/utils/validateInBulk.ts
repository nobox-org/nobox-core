...
export const validateInBulk = (
   validation: ClassValidatorType | ClassValidatorType[],
   valueObj: Record<any, any>,
) => {
   ...
   for (let index = 0; index < values.length; index++) {
      ...
      if (Array.isArray(validation)) {
         ...
         assertValidation(validation, value, name);
      }
   }
};
