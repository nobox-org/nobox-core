...
export class ClientServiceMongoSyntaxUtil {
    ...
    > {
        ...
        if (recordDocument) {
            ...
            this.logger.sLog({ preparedCommand, errors }, "ClientServiceMongoSyntaxUtil::createSyntax::preparedCommand");
    ...
    private async _createRecordQuerySyntax(
        ...
        this.logger.sLog({ recordSpaceSlug, recordSpaceId, query, recordFields, acrossRecords }, "ClientServiceMongoSyntaxUtil::prepareRecordQuery");
    ...
    private async _createRecordCommandSyntax(recordSpaceId: string, body: Record<string, string>, recordFields: MRecordField[], requiredFieldsAreOptional = false) {
        this.logger.sLog({ recordFields, recordSpaceId, body }, "ClientServiceMongoSyntaxUtil::createRecordCommandSyntax")
        ...
        this.logger.sLog({ preparedCommand }, "ClientServiceMongoSyntaxUtil::createRecordCommandSyntax::result");

        return { preparedCommand };
    }

    private async _createDocumentByField(fieldDetails: MRecordField, value: string) {
        this.logger.sLog({ fieldDetails, value }, "ClientServiceMongoSyntaxUtil::_createDocumentByField")
    ...
    private _initializeQuery(recordSpaceId: string, query: Record<string, string>, acrossRecords: boolean) {
        this.logger.sLog({ recordSpaceId, query, acrossRecords }, "ClientServiceMongoSyntaxUtil::_initializePreparedQuery");
        const preparedQuery = {
            recordSpace: String(recordSpaceId),
    ...
    async validatingSearchableText(args: {
        ...
        this.logger.sLog({ searchableFields: args.searchableFields }, 'ClientServiceMongoSyntaxUtil::validatingSearchableText');
        ...
        if (searchableFields.length) {
            ...
            if (invalidFields.length) {
                this.logger.sLog({ invalidFields, recordSpaceSlug }, 'ClientService::validatingSearchableText::invalidFields');
            ...
            this.logger.sLog({ searchableFieldsIsDifferent, searchableFieldsIsDifferentByLength, newSearchableFields, "existingSearchableFieldsInRecordSpace": recordSpace.searchableFields }, "ClientService::validatingSearchableText")

            if (searchableFieldsIsDifferent) {
                this.logger.sLog({ searchableFields, recordSpaceSlug }, 'ClientService::validatingSearchableText:: updated searchableFields on recordSpace');
