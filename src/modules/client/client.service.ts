...
import { CObject, CommandType, Context, ClientSourceFunctionType, HydratedRecordSpace, ParamRelationship, TraceObject } from '@/types';
...
export class ClientService {
    ...
    async getRecords(args: {
        ...
        this.logger.sLog({ recordQuerySyntax, formattedRecordQuery }, 'ClientService::getRecords::recordQuerySyntax');
        ...
        if (records.length === 0) {
            this.logger.debug(`No records found for project : ${projectSlug}, recordSpaceSlug: ${recordSpaceSlug}`, 'ClientService::getRecords');
            return [];
        }
    ...
    async searchRecords(args: {
        ...
        if (skipPreOperation && !options.recordSpace) {
            this.logger.sLog({ args, options }, 'ClientService::searchRecords::skipPreOperation is true but recordSpace is not provided');
        ...
        if (!skipPreOperation) {
            if (!args.params) {
                this.logger.sLog({ args }, 'ClientService::searchRecords::params not found in args');
                throwBadRequest('Something went wrong');
            }
    ...
    async getRecordById(
        ...
        if (skipPreOperation && !options?.resources) {
            this.logger.sLog({ args, options }, 'ClientService :: getRecordById :: skipPreOperation is true but options resources  is not provided');
            throwBadRequest('Something went wrong');
        }
        ...
        if (!record) {
            this.logger.debug(`No records found for project: ${projectSlug}, recordSpaceSlug: ${recordSpaceSlug}`, 'ClientService:getRecord')
            if (throwOnEmpty) {
                throwBadRequest(
    ...
    async getTokenOwner(
        ...
        if (!user) {
            this.logger.sLog({ user }, "ClientService::_getTokenOwner:: token user does not exist");
    ...
    async getRecord(
        ...
        if (!skipPreOperation) {
            ...
            if (!args.params) {
                this.logger.sLog({ args }, 'ClientService::getRecord::params not found in args');
                throwBadRequest('Something went wrong');
            }
        ...
        if (!record) {
            this.logger.debug(`No record found for project: ${projectSlug}, recordSpaceSlug: ${recordSpaceSlug}`, 'ClientService:getRecord')
    ...
    async addRecord(args: {
        ...
        if (errors) {
            this.logger.sLog({ errors }, 'ClientService:addRecord:: while creating syntax');
            throwBadRequest(errors);
        }
    ...
    async updateRecordById(args: {
        ...
        this.logger.sLog(
            args,
            'ClientService::updateRecordById',
        );
        ...
        if (!ObjectId.isValid(id)) {
            ...
            this.logger.sLog({ id }, "ClientService:updateRecordById:: invalid id")
        }
    ...
    async getKeyValues(
        ...
        if (!record) {
            this.logger.debug(`No records found for project: ${projectSlug}, recordSpaceSlug: ${recordSpaceSlug}`, 'ClientService:getKeyValues')
            if (throwOnEmpty) {
                throwBadRequest(
                    `No record found for your request`,
                );
        ...
        if (record.length > 1) {
            this.logger.debug(`More than one records found for project: ${projectSlug}, recordSpaceSlug: ${recordSpaceSlug}`, 'ClientService:getKeyValues')
            throwBadRequest(
    ...
    async setKeyValues(args: {
        ...
        if (existingRecordUpdate.length > 1) {
            this.logger.sLog({ existingRecordUpdate, recordSpaceSlug }, `ClientService::setKeyValues::Multiple records found for Key-based for ${recordSpaceSlug}`);
        }
    ...
    private async createKeyValueRecord(args: {
        ...
        if (!existingRecord) {
            this.logger.sLog({ recordSpaceSlug }, "ClientService::updateRecord::First Record for Key-based Value");
            const record = await this.recordsService.create(
                {
                    ...recordCommandSyntax,
                    recordSpaceSlug,
    ...
    async preOperationMutation(
        ...
        if (!initialData) {
            this.logger.sLog({ params }, "ClientService:preOperationMutation:: no initial data was sent");
            return;
        }

        this.logger.sLog({ initialDataExist }, "ClientService:preOperationMutation");
    ...
    async preOperationClearing(args: {
        ...
        this.logger.sLog({ mutate, clearAllRecordSpaces, clearThisRecordSpace }, "ClientService::preOperationClearing");

        if (clearAllRecordSpaces) {
            await this.recordsService.deleteAllRecordsInProject(project.slug, String(project._id));
            this.logger.sLog({}, "ClientService::preOperation::clearAllRecordSpaces:: Cleared all records from all recordSpaces");
        }
        ...
        if (!clearAllRecordSpaces && clearThisRecordSpace) {
            ...
            this.logger.sLog({ res, recordSpaceSlug: recordSpace.slug }, `ClientService::preOperation::clearAllRecords:: Cleared ${res[0].deletedCount} records from ${recordSpace.slug}`);
            if (!initialData) {
    ...
    async preOperationDataInitialization(args: {
        ...
        this.logger.sLog({ params: args.params, initialData: args.operationResources.initialData }, "ClientService::preOperationDataInitialization")
        const { params, operationResources } = args;
        const { recordSpace, initialData } = operationResources;

        this.logger.sLog({ recordSpace }, "ClientService::preOperationDataInitialization:: filling initialData");
    ...
    private async processSpaceAuthorization(
        ...
        if (!scope.includes(commandType as any)) {
            this.logger.sLog({ commandType, authOptions }, "ClientService::_assertSpaceAuthorization:: authorization Skipped for commandType");
            return;
        }

        if (scope.includes(commandType as any)) {
            if (!token) {
                this.logger.sLog({ token }, "ClientService::_assertSpaceAuthorization:: token is missing");
                this.logger.sLog({ commandType, authOptions }, "ClientService::_assertSpaceAuthorization:: commandType not allowed without authorization");
            ...
            if (!tokenRes) {
                this.logger.sLog({ token }, "ClientService::_assertSpaceAuthorization:: invalid token");
            ...
            if (!userId) {
                this.logger.sLog({ userId }, "ClientService::_prepareOperationResources:: invalid token details");
            ...
            if (!recordSpace) {
                this.logger.sLog({ recordSpace }, "ClientService::_prepareOperationResources:: authOptions recordSpace does not exist")
            ...
            if (!userIdExist) {
                this.logger.sLog({ userIdExist }, "ClientService::_prepareOperationResources:: token user does not exist");
                throwBadRequest(`Token user does not exist`);
            }
        }
    }
    ...
    private async _prepareOperationResources(
        ...
        if (!isSearch) {
            ...
            if (typeErrors.length) {
                this.logger.sLog({ typeErrors }, "ClientService::_prepareOperationResources:: typeErrors ocurred")
        ...
        if (!requestIsAQuery && isEmpty(fieldsToConsider)) {
            this.logger.sLog({ query, body }, "ClientService:preOperation:: Both query and body parameters are empty");
        ...
        this.logger.sLog({ resources }, "ClientService::_prepareOperationResources:: resources");
    ...
    private async processSkipPreOperation(args: {
        ...
        if (skipPreOperation && !options.recordSpace) {
            this.logger.sLog({ args, options }, 'ClientService::getRecords::processSkipPreOperation is true but recordSpace is not provided');
            throwBadRequest('Something went wrong');
        }
        ...
        if (!skipPreOperation) {
            if (!params) {
                this.logger.sLog({ args }, 'ClientService::processSkipPreOperation::params not found in args');
                throwBadRequest('Something went wrong');
            }
