...
import { MProject, getRecordSpaceModel, getRecordFieldModel, MRecordField, MRecordSpace } from '@/schemas';
import { Endpoint, GenerateEndpointInput, RecordStructure } from './types';
...
export class RecordSpacesService {
  ...
  */
  ...
  async getEndpoints(recordSpaceDetails?: GenerateEndpointInput): Promise<Endpoint[]> {
  ...
  async shouldUpdateRecordSpace(args: {
    ...
    if (!allowMutation && mutationIsRequired) {
      this.logger.sLog({ allowMutation }, "ClientService::handleRecordSpaceCheckInPreOperation:: mutation is not allowed");
  ...
  async handleRecordSpaceMutationInPreOperation(args: {
    ...
    if (!recordSpace) {
      this.logger.sLog({ recordSpace }, "ClientService::_prepareOperationResources:: recordSpace or project does not exist");
      ...
      if (!autoCreateRecordSpace) {
        this.logger.sLog({ autoCreateRecordSpace: autoCreateRecordSpace }, "ClientService::autoCreateRecordSpace:: auto creating recordSpace not allowed");
