...
export class ClientFunctionsService {
   ...
   async sendPushNotification(
      args: Omit<ClientFunctionsEndpointsDataResponse, 'functionName'>,
   ) {
      ...
      const latestProject = await this.projectsService.findOne({
         _id: project._id,
      });
      ...
      if (!keys) {
         this.logger.sLog(
            {},
            'ClientFunctionsService::sendPushNotification::No API keys are found',
         );
         throwBadRequest(
            'No API keys are found, Please update your project keys',
         );
      }
      ...
      if (!firebase) {
         this.logger.sLog(
            {},
            'ClientFunctionsService::sendPushNotification::No Firebase keys were found',
         );
      ...
      if (!findBy && !fcmToken) {
         this.logger.sLog(
            {},
            'ClientFunctionsService::sendPushNotification::No findBy parameter is found, and no fcmToken is found',
         );
         throwBadRequest(
            'No findBy parameter is found, and no fcmToken is found, please set one of them',
         );
      }
      ...
      if (findBy) {
         ...
         if (!record) {
            this.logger.sLog(
               {},
               'ClientFunctionsService::sendPushNotification::No record is found using findBy parameter',
            );
         ...
         if (!fcmToken) {
            this.logger.sLog(
               {},
               'ClientFunctionsService::sendPushNotification::No fcmToken is found in the chosen space record',
            );
      ...
      this.logger.sLog(
         { fcmToken, title, body },
         'ClientFunctionsService::sendPushNotification::Sending notification',
      );
      ...
      this.logger.sLog(
         { response },
         'ClientFunctionsService::sendPushNotification::Notification sent',
      );
   ...
   async sendOtp(
      ...
      if (!keys) {
         this.logger.sLog(
            {},
            'ClientFunctionsService::sendOtp::No API keys are found',
         );
      ...
      if (!postmark) {
         this.logger.sLog(
            {},
            'ClientFunctionsService::sendOtp::No Postmark API key is found',
         );
      ...
      const {
         ...
         businessDetails: {
            ...
            address: projectBusinessAddress,
         },
      } = project;
      ...
      utils
         ...
         .catch((error: any) => {
            this.logger.sLog(
               { error },
               'ClientFunctionsService::sendOtp::Error sending email',
            );
         });
   ...
   async login(
      ...
      this.logger.sLog(args, 'ClientFunctionsService::login');
   ...
   ): Promise<ClientFunctionsEndpointsDataResponse> {
   ...
   private async _prepareOperationDetails(
      ...
      } = this.contextFactory.getValue(['headers']);

      const user = this.contextFactory.getValue(['user']);

      const {
         mustExistSpaceStructures: [incomingRecordSpaceStructure, ..._],
      } = functionResources;
      ...
      if (projectSlugOnParam !== projectSlugOnStructure) {
         this.logger.sLog(
            { projectSlugOnParam, projectSlugOnStructure },
            'ClientFunctionsEndpoints::preOperation:: mismatched projectSlug on param and structure',
         );
         throwBadRequest(
            'Project Slug on Param and SpaceStructure is different',
         );
      }
      ...
      if (errors.length) {
         this.logger.sLog(
            { errors },
            'ClientFunctionsService::PreOperation: throw validation Error',
         );
