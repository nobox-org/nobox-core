...
export const sendEmail = async (args: {
   ...
   logger.sLog(
      { recipient, templateType, variables, customSubject, apiKey },
      'utils::email::service::sendEmail',
   );
   ...
   try {
      const {
         ...
         htmlContent,
         textContent,
      } = await getDetailsFromTemplate(templateType, variables);
      logger.debug(
         `Sending Email ${JSON.stringify({ to: recipient.email, subject })}`,
      );
      const postMarkMailCarrier = new postmark.ServerClient(apiKey);
      logger.sLog(
         { sender, recipient, subject, textContent, htmlContent, variables },
         'utils::email::service::sendEmail: about to run postmark request',
      );
